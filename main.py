...
def Load_Data():
    ...
    if config.FIVE_CLASS_TASK:
        ...
        # if config.CNN_MUI is True or config.DEEP_CNN_MUI is True:
        #     train_iter, dev_iter = mrs_five_mui(config.datafile_path, config.name_trainfile, config.name_devfile, config.name_testfile, config.char_data, text_field=config.text_field, label_field=config.label_field,
        ...
        # else:
        #     train_iter, dev_iter = mrs_five(config.datafile_path, config.name_trainfile, config.name_devfile, config.name_testfile, config.char_data,
        ...
        train, valid = data.TabularDataset.splits(path='Data', train='train.csv',
                                                          validation='valid.csv',
                                                          format='csv',
                                                          skip_header=True,
                                                          fields=[('text', config.text_field),
                                                                  ('label', config.label_field)])
        config.text_field.build_vocab(train, valid)
        config.label_field.build_vocab(train, valid)
        if config.CNN_MUI is True or config.DEEP_CNN_MUI is True:
            train_iter, dev_iter = data.Iterator.splits((train, valid),
                                                        batch_sizes=(
...
def define_dict():
    ...
    config.text_field = data.Field(sequential=True, tokenize=tokenizer, fix_length=150)
    config.label_field = data.Field(sequential=False)
    config.static_text_field = data.Field(sequential=True, tokenize=tokenizer, fix_length=150)
...
def start_train(model, train_iter, dev_iter):
    ...
    #         print("\nSorry. The test dataset doesn't  exist.\n")
        ...
        if config.CNN:
            ...
            model_count = train_ALL_CNN.train(train_iter, dev_iter, model, config)
        elif config.DEEP_CNN:
            print("DEEP_CNN training start......")
            model_count = train_ALL_CNN.train(train_iter, dev_iter, model, config)
        elif config.LSTM:
            print("LSTM training start......")
            model_count = train_ALL_LSTM.train(train_iter, dev_iter, model, config)
        elif config.GRU:
